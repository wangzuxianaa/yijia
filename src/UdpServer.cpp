...
void UdpServer::RecvJsonData()
{
    ...
    int r = recvfrom(sockfd, RecvBuff, 1024, 0, (sockaddr*) &client, &client_len);
    ...
    if(r < 0) {
        ...
        if(reader.parse(RecvBuff, val)) {
            ...
            if(val["Button_Stop"].asInt() == 0) {
                ...
                jsonwriter["Twist"].append(AGV_Info.Vz);
                SendJsonData(jsonwriter);
            }
            else if(val["Angular_Vel"] == 0) {
            ...
            else if(val["Linear_Acc"].asInt() == 0) {
